...
class Model(nn.Module):
    ...
    def forecast(self, x_enc):
        ...
        BN, pred_len, out_dim = dec_out.shape
        B = BN // self.configs.enc_in  # 恢复真实的batch size
        dec_out = dec_out.reshape(B, self.configs.enc_in, pred_len, out_dim)  # [B, N, pred_len, 1]

        # 根据任务类型选择输出
        if self.configs.features == 'MS' and self.configs.c_out == 1:
            dec_out = dec_out[:, -1:, :, :]  # 选择最后一个特征(target)

        dec_out = dec_out.squeeze(-1)  # 移除最后一个维度: [B, N, pred_len] 或 [B, 1, pred_len]

        # 确保输出维度正确
        if dec_out.shape[1] > self.configs.c_out:
            dec_out = dec_out[:, :self.configs.c_out, :]  # [B, c_out, pred_len]

        dec_out = dec_out.permute(0, 2, 1).contiguous()  # [B, pred_len, c_out]
